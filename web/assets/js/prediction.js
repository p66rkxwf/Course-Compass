    ...
    return `
        <div class="admission-detail ${r.cls}">
            ...
            <div class="small text-muted mt-2">
                ...
                區間在新學期常偏窄（115-1 實測涵蓋約 70%），請當作參考而非保證。
            </div>
        </div>`;
}
...
export async function renderModelInfo(containerId = 'model-info-body') {
    ...
    try {
        const res = await fetch(`${API_BASE}/predict/model-info`);
        ...
        el.innerHTML = `
            ...
            ${info.holdout ? table(`保留學期 ${info.holdout.semester}（模型凍結後只評估一次）`, info.holdout.results) : ''}
            <p class="mb-0 text-muted">PR-AUC 越高越能辨認會爆滿的課；中籤率誤差越低越準。完整報告見 <code>docs/demand_model_report.md</code>。</p>`;
        el.dataset.loaded = '1';
    } catch (e) {
        el.innerHTML = '<span class="text-muted">尚未訓練中籤預測模型（執行 <code>python main.py train-demand</code>）。</span>';
    }
}
