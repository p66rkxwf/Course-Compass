...
def main():
    ...
    parser.add_argument(
        ...
        choices=["crawl", "process", "build-dict", "api", "all",
                 "train-demand", "eval-holdout", "fetch-syllabi", "build-index", "eval-agent", "eval-rag"],
        help="要執行的命令"
    )
    parser.add_argument(
        "--semesters", nargs="+", metavar="YYY-S",
        help="crawl：只爬指定學期，例如 --semesters 114-2 115-1"
    )
    parser.add_argument(
