...
- 沒有預測的新學期（例如選課前剛公告的 115-2）：用凍結模型現算，特徵同樣只用過去學期
"""
...
class PredictionStore:
    ...
    def _ensure_semester(self, year: str, semester: str, full_df: Optional[pd.DataFrame]) -> None:
        ...
        sem = (year, semester)
        if sem in self._computed_semesters or full_df is None:
            return
        self._computed_semesters.add(sem)
        if any(k[:2] == sem for k in self._preds):
            return
        model_path = self.models_dir / "demand_model.joblib"
        if not model_path.exists():
            return
        try:
            import joblib
            from ml.demand_features import KEY, build_course_table, build_features, label_to_idx

            if self._model is None:
                self._model = joblib.load(model_path)
            courses = build_course_table(full_df)
